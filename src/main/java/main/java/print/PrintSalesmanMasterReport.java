...
public class PrintSalesmanMasterReport {
    public static String filename = "D:\\Software\\Prints\\SalesmanMaster.pdf";
    ...
    float commision=0,paid=0;
    public PrintSalesmanMasterReport(List<Commision>list, LocalDate start,LocalDate end)
    {
        this.list = list;
        this.start = start;
        this.end = end;
    ...
    private void addData(Document doc) throws DocumentException {
        ...
        c1 = new PdfPCell(new Paragraph("Salesman Name:"
                +list.get(0).getEmployee().getFname()+" "
                +list.get(0).getEmployee().getMname()+" "
                +list.get(0).getEmployee().getLname(), subhead));
        ...
        float[] columnWidths = new float[] { 5f, 10f, 10f, 10f,10f, 20f};
    ...
    private void addData(PdfPTable tr)
    {
        int sr=0;
        for(Commision com:list)
        {
            for(CommisionTransaction ctr:com.getTransaction())
            {
                ctr.setId(++sr);
                addRow(ctr,tr);
            }
        ...
        c1 = new PdfPCell(new Paragraph(""+paid ,footerFont));
        c1.setHorizontalAlignment(Element.ALIGN_LEFT);
        c1.setBorder(PdfPCell.BOX);
        c1.setColspan(2);
        tr.addCell(c1);
    ...
    void addRow(CommisionTransaction ctr,PdfPTable tr)
    {
        PdfPCell c1;
        c1 = new PdfPCell(new Paragraph(""+ctr.getId() ));
        c1.setHorizontalAlignment(Element.ALIGN_LEFT);
        c1.setBorder(PdfPCell.BOX);
        tr.addCell(c1);

        c1 = new PdfPCell(new Paragraph(""+ctr.getDate() ));
        c1.setHorizontalAlignment(Element.ALIGN_LEFT);
        c1.setBorder(PdfPCell.BOX);
        tr.addCell(c1);

        c1 = new PdfPCell(new Paragraph(""+ctr.getBill().getBillno() ));
        c1.setHorizontalAlignment(Element.ALIGN_LEFT);
        c1.setBorder(PdfPCell.BOX);
        tr.addCell(c1);

        c1 = new PdfPCell(new Paragraph(""+ctr.getTotalCommision()));
        c1.setHorizontalAlignment(Element.ALIGN_LEFT);
        c1.setBorder(PdfPCell.BOX);
        tr.addCell(c1);

        c1 = new PdfPCell(new Paragraph(""+ctr.getPaidCommision()));
        c1.setHorizontalAlignment(Element.ALIGN_LEFT);
        c1.setBorder(PdfPCell.BOX);
        tr.addCell(c1);

        c1 = new PdfPCell(new Paragraph(""+ctr.getCommision().getBank().getBankname()));
        c1.setHorizontalAlignment(Element.ALIGN_LEFT);
        c1.setBorder(PdfPCell.BOX);
        tr.addCell(c1);

        commision+=ctr.getTotalCommision();
        paid+=ctr.getPaidCommision();

    }
    private void addHeader(PdfPTable tr) {
        PdfPCell c1;
        ...
        c1 = new PdfPCell(new Paragraph("Commission" ));
        c1.setHorizontalAlignment(Element.ALIGN_LEFT);
        c1.setBorder(PdfPCell.BOX);
        tr.addCell(c1);

        c1 = new PdfPCell(new Paragraph("Paid Commision" ));
        c1.setHorizontalAlignment(Element.ALIGN_LEFT);
        c1.setBorder(PdfPCell.BOX);
        tr.addCell(c1);
